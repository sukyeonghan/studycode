...
//패키지, 클래스명
// 필드,멤버 변수
//-,프라이빗 : 접근제한자. 접근할 수 있는 범위를 정해 놓음
//밑에 기능(메소드)

//new연산자로 클래스생성하면 객체가 생김
//인스턴스화 : 코드를,변수와 변수의 자료공간을 실질적으로 쓸 수 있게 만든 것.설계도대로 만들었다.
//캡슐화 : 지정한 데이터만 받을 수 있게 차단. 숨겨놓는 것. 프라이빗을 사용. 기능으로 통제함
...
//이것도 마찬가지임! 클래스명().메소드명()
//기능가지고 있는 클래스/데이터 담을 수 있는 클래스로 나눌 수 있음
//클래스를 기반으로 객체를 생성하고 저장공간에 있는 변수를 사용..?

//클래스 설계- 객체 생성 방법
//클래스-설계도면 역할
//상속 : 객체 간 관계를 맺어줘서 쓸 수 있게 허용.미리 만들어 놓은 클래스 가져다쓰는 거. 
//다형성 : 자료형이 다른데 여러 명 자식객체를 부모객체한테 넣을 수 있음
